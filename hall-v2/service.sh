#!/system/bin/sh
# TB375FC: hall_status 1=open, 0=closed. Settings toggle: Global.device_case_enabled.
MODDIR=${0%/*}
...
setting_enabled() { [ "$(settings get global device_case_enabled 2>/dev/null)" = 1 ]; }
...
sync_state() {
    ...
    if ! setting_enabled; then
        message "$label ignored: Settings switch is off"
        return
    fi
...
if [ "${1:-}" = --self-test ]; then
    read_state() { printf '%s\n' "$TEST_STATE"; }
    setting_enabled() { [ "$TEST_ENABLED" = 1 ]; }
    message() { :; }
    input() { [ "$FAIL_INPUT" = 1 ] && return 1; ACTIONS="$ACTIONS $2"; }
    ACTIONS=; FAIL_INPUT=0; TEST_ENABLED=0; TEST_STATE=0
    on_event 0001 00fc 00000001
    TEST_STATE=1; on_event 0001 00fc 00000001
    [ -z "$ACTIONS" ] || exit 1
    TEST_ENABLED=1; TEST_STATE=0; on_event 0001 00fc 00000001
    TEST_STATE=1; on_event 0001 00fd 00000001
    on_event 0001 00fc 00000000
    [ "$ACTIONS" = ' KEYCODE_SLEEP KEYCODE_WAKEUP' ] || exit 1
    TEST_STATE=0; FAIL_INPUT=1; on_event 0001 00fc 00000001
    FAIL_INPUT=0; on_event 0001 00fc 00000001
    [ "$ACTIONS" = ' KEYCODE_SLEEP KEYCODE_WAKEUP KEYCODE_SLEEP' ] || exit 1
    echo 'PASS: Settings off suppresses actions; on handles close/open and failed-action retry'
    exit 0
fi
...
while :; do
    ...
    READER=$!
    exec 4<"$FIFO"
    message "Listening on $DEVICE; switch=global.device_case_enabled"
    LAST_STATE=
    sync_state
