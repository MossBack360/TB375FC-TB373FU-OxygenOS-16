...
def main():
    ...
    overlay = "--system-overlay" in sys.argv
    output = ROOT / ("out/Settings-fixed-system-overlay.apk" if overlay else "out/Settings-fixed-unsigned.apk")
    with zipfile.ZipFile(ROOT / "extract/Settings.apk") as original, zipfile.ZipFile(output, "w") as patched:
        for entry in original.infolist():
            if entry.filename in SIGNATURES and not overlay:
                continue
