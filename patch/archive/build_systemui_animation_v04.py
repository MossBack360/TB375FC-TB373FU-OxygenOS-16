"""Build the combined caption, corner-transition, and back-gesture module."""
...
DEST = ROOT / "out/systemui-animation-polish-v0.4-2026-09-11"
...
def patch_classes2(raw: bytes) -> bytes:
    ...
    min_x_field = dex.field(spring, "mMinScaleXInMove", "F")
    min_y_field = dex.field(spring, "mMinScaleYInMove", "F")
    radius_field = dex.field(spring, "mRadius", "F")
    ...
    math_max = dex.method("Ljava/lang/Math;", "max", "(FF)F")
    math_min = dex.method("Ljava/lang/Math;", "min", "(FF)F")
    ...
    old_curve = (
        struct.pack("<BBH", 0x52, 0x0C, min_x_field)
        + struct.pack("<BBH", 0x52, 0x0D, min_y_field)
        + invoke_35c(0x71, math_max, [12, 13])
        + bytes((0x0A, 0x0C, 0xC8, 0x4C))
        + invoke_35c(0x71, math_max, [10, 8])
        + bytes((0x0A, 0x0D, 0xC8, 0xD4))
        + invoke_35c(0x71, math_max, [4, 12])
        + bytes((0x0A, 0x04))
        + struct.pack("<BBH", 0x59, 0x04, radius_field)
    )
    # R = min(50px, R_fullscreen + (1 - max(scaleX, scaleY)) * 200).
    # TB375FC uses 30px at fullscreen and reaches 50px at the 0.9 enter threshold.
    new_curve = (
        invoke_35c(0x71, math_max, [10, 8])
        + bytes((0x0A, 0x0D))
        + struct.pack("<BBH", 0x15, 0x0C, 0x3F80)  # const/high16 v12, 1.0f
        + bytes((0xC7, 0xDC))  # sub-float/2addr v12, v13
        + struct.pack("<BBH", 0x15, 0x0D, 0x4348)  # const/high16 v13, 200.0f
        + bytes((0xC8, 0xDC, 0xC6, 0xC4))  # mul v12,v13; add v4,v12
        + struct.pack("<BBH", 0x15, 0x0C, 0x4248)  # const/high16 v12, 50.0f
        + invoke_35c(0x71, math_min, [4, 12])
        + bytes((0x0A, 0x04))
        + struct.pack("<BBH", 0x59, 0x04, radius_field)
        + bytes((0x00, 0x00))
    )
    curve_pos = dex.replace_once(old_curve, new_curve, "fullscreen-to-freeform radius curve")
    return dex.finish(raw, [(fallback_pos, len(old_fallback)), (curve_pos, len(old_curve))])


def patch_classes4(raw: bytes) -> bytes:
    dex = DexTables(raw)
    seek = "Lcom/oplus/flexibletask/seekbar/FlexibleSeekBarManager;"
    holder = "Lcom/oplus/flexibletask/animation/SurfaceAnimValueHolder;"
    handler = "Lcom/android/wm/shell/flexibletask/FlexibleTaskTransitionHandler;"
    bounds = "Lcom/android/wm/shell/flexibletask/FlexibleTaskTransitionHandler$AnimationBounds;"
    base_config = "Lcom/android/wm/shell/flexibletask/FlexibleTaskTransitionHandler$BaseAnimationConfig;"
    txn = "Landroid/view/SurfaceControl$Transaction;"
    task = "Landroid/app/ActivityManager$RunningTaskInfo;"
    surface = "Landroid/view/SurfaceControl;"
    get_seek_alpha = dex.method(seek, "getAlpha", f"({task})F")
    create_leash_holder = dex.method(handler, "createMaximizedLeashAnimationHolder", f"({bounds}F){holder}")
    create_screenshot_holder = dex.method(handler, "createScreenshotAnimationHolder", f"(ZFF){holder}")
    set_holder_alpha = dex.method(holder, "setAlpha", f"(FF){holder}")
    get_holder_alpha = dex.method(holder, "getAlpha", "()F")
    set_surface_alpha = dex.method(txn, "setAlpha", f"({surface}F){txn}")
    set_corner = dex.method(txn, "setCornerRadius", f"({surface}F){txn}")
    leash_field = dex.field(base_config, "mLeashLayer", surface)

    # In createMaximizedAnimationConfig, feed 50px into the leash holder instead
    # of the seek-bar alpha. The screenshot holder still receives the real alpha.
    old_start = (
        invoke_35c(0x6E, get_seek_alpha, [11, 5])
        + bytes((0x0A, 0x0B))
        + invoke_35c(0x70, create_leash_holder, [10, 14, 11])
        + bytes((0x0C, 0x05, 0x12, 0x06))
        + invoke_35c(0x70, create_screenshot_holder, [10, 3, 11, 6])
    )
    new_start = (
        struct.pack("<BBH", 0x15, 0x0B, 0x4248)  # p1 = 50px for the leash radius holder
        + invoke_35c(0x70, create_leash_holder, [10, 14, 11])
        + bytes((0x0C, 0x05))
        + struct.pack("<BBH", 0x15, 0x0B, 0x3F80)  # restore p1 = 1.0 for screenshot alpha
        + bytes((0x12, 0x06))
        + invoke_35c(0x70, create_screenshot_holder, [10, 3, 11, 6])
    )
    start_pos = dex.replace_once(old_start, new_start, "maximize radius start")

    # In createMaximizedLeashAnimationHolder, animate the reused holder property
    # from 50px to the tablet fullscreen radius, 30px.
    old_end = struct.pack("<BBH", 0x15, 0x08, 0x3F80) + invoke_35c(0x6E, set_holder_alpha, [7, 9, 8])
    new_end = struct.pack("<BBH", 0x15, 0x08, 0x41F0) + invoke_35c(0x6E, set_holder_alpha, [7, 9, 8])
    end_pos = dex.replace_once(old_end, new_end, "maximize radius end")

    # In LeashUpdateStrategy.onUpdate, apply that animated value as corner radius.
    apply_prefix = struct.pack("<BBH", 0x54, 0x88, leash_field)  # iget-object v8, v8
    old_apply = apply_prefix + invoke_35c(0x6E, set_surface_alpha, [7, 8, 2])
    new_apply = apply_prefix + invoke_35c(0x6E, set_corner, [7, 8, 2])
    apply_pos = dex.replace_once(old_apply, new_apply, "maximize radius application")
    return dex.finish(raw, [(start_pos, len(old_start)), (end_pos, len(old_end)), (apply_pos, len(old_apply))])
...
def main() -> None:
    ...
    output = DEST / "SystemUI-animation-polish-v0.4.apk"
    with ZipFile(SOURCE) as source:
        replacements = {
            "classes2.dex": patch_classes2(source.read("classes2.dex")),
            "classes4.dex": patch_classes4(source.read("classes4.dex")),
        ...
        with ZipFile(output) as result:
            ...
            assert changed == ["classes2.dex", "classes4.dex", "classes6.dex"], changed

    module = DEST / "fixo-systemui-animation-polish-v0.4-magisk.zip"
    service = (ROOT / "out/fullscreen-caption-confirmed-2026-09-09/service.sh").read_bytes().replace(b"\r\n", b"\n")
    entries = {
        "module.prop": (
            "id=fixo_fullscreen_caption\n"
            "name=TB375FC OxygenOS SystemUI animation polish\n"
            "version=0.4-test\n"
            "versionCode=4\n"
            "author=FixO\n"
            "description=Enable fullscreen caption controls, smooth task corner handoff, and restore follow-finger back gesture animation.\n"
        ).encode(),
        "service.sh": service,
    ...
    (DEST / "README.md").write_text(
        "# OxygenOS SystemUI animation polish v0.4\n\n"
        ...
        "- Interpolates 30 px to the 50 px flexible-window radius before handoff.\n"
        "- Reuses the maximize spring to interpolate 50 px back to 30 px when returning fullscreen.\n"
        ...
        "- Only `classes2.dex`, `classes4.dex`, and `classes6.dex` differ from the confirmed caption SystemUI.\n",
        encoding="utf-8",
    )
